...
echo "🔍 CIS KB DATA VALIDATION & QUALITY AUDIT v2.1\n";
echo str_repeat("=", 60) . "\n";
...
try {
    ...
    echo "   • Average length: " . number_format((float)$contentStats['avg_length']) . " characters\n";
    ...
    // Check keyword quality with proper type handling
    $keywordStats = $pdo->query("
        SELECT 
            ...
            AVG(CAST(content_complexity as DECIMAL(5,2))) as avg_complexity
        FROM ecig_kb_files
    ")->fetch();
    ...
    echo "   • Average complexity: " . round((float)($keywordStats['avg_complexity'] ?? 0), 1) . "\n";
...
try {
    ...
    echo "   • Avg content length: " . number_format((float)$searchStats['avg_content_length']) . " chars\n";
    echo "   • Avg terms length: " . number_format((float)$searchStats['avg_terms_length']) . " chars\n";
...
try {
    ...
    foreach ($cognitiveStats as $cog) {
        ...
        $typeGroups[$type]['confidence'] += (float)$cog['avg_confidence'] * $cog['count'];
        $typeGroups[$type]['value'] += (float)$cog['avg_business_value'] * $cog['count'];
...
try {
    ...
    foreach ($fileTypeStats as $type) {
        ...
        $avgSize = number_format((float)$type['avg_size']);
        $avgLines = number_format((float)$type['avg_lines']);
