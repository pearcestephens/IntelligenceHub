...
class AIOrchestrator {
    ...
    private function identifyRequiredTools(string $message, array $intent): array {
        ...
        // If asking about knowledge base structure
        if (strpos($message, 'knowledge base') !== false || strpos($message, 'what do you know') !== false) {
            $needed[] = 'kb_scanner';
        }
        
        // If asking about database
        if (!empty($intent['entities']['tables'])) {
            $needed[] = 'db_query';
        }
        
        // If asking for file content
        if (strpos($message, 'file') !== false || strpos($message, 'code') !== false) {
            $needed[] = 'file_reader';
        }
    ...
    private function enhancedKeywordSearch(string $message, array $intent): array {
        ...
        // Search with different strategies based on intent
        if ($intent['primary'] === 'explain') {
            // Prioritize documentation and guides
            $contexts = $this->searchByCategory($keywords, ['documentation', 'guide', 'tutorial']);
        } elseif ($intent['requires_data']) {
            // Prioritize code and database schemas
            $contexts = $this->searchByCategory($keywords, ['code', 'schema', 'api']);
        } else {
            // General search
    ...
    private function buildSystemPrompt(array $intent): string {
        $base = "You are an expert AI assistant for The Vape Shed CIS (Central Information System).";
        
        if ($intent['primary'] === 'explain') {
            return $base . " Focus on providing clear, educational explanations with examples.";
        } elseif ($intent['requires_data']) {
            return $base . " Provide data-driven answers with specific numbers and references.";
        } elseif (in_array('analyze', $intent['all'])) {
            return $base . " Analyze the data critically and provide insights and recommendations.";
        }
        
        return $base . " Provide accurate, helpful responses based on the knowledge base.";
    }
