<?php
/**
 * File: public/api/HttpTool.php
 * Purpose: HTTP wrapper bridging MCP http.request calls into App\Tools\HttpTool
...
$toolName = isset($payload['tool']) && is_string($payload['tool']) ? $payload['tool'] : 'http.request';
...
try {
    $result = HttpTool::run($arguments, $context);
} catch (\Throwable $exception) {
    ApiBootstrap::error('HTTP tool execution failed', 500, 'tool_error', [
