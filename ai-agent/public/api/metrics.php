...
/**
 * Metrics Endpoint - SIMPLIFIED (No Bootstrap)
 * 
 * Returns system metrics for monitoring dashboards and alerting.
 * Direct checks without framework overhead for reliability.
 * 
 * @package App\API
    ...
    if (extension_loaded('redis')) {
        ...
        if ($redis->connect('127.0.0.1', 6379, 1.0)) {
            ...
            try {
                ...
                $cbStates = [];
                foreach (['openai', 'claude', 'database'] as $service) {
                    $stateKey = 'aiagent:circuit:' . $service . ':state';
                    $state = $redis->get($stateKey);
                    $cbStates[$service] = $state ? $state : 'closed';
                }
                $metrics['metrics']['circuit_breakers'] = $cbStates;
            } catch (Exception $e) {
