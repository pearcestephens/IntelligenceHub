...
class ScheduleOptimizer
{
    ...
    /**
     * Generate optimized schedule based on task definitions from tasks.json
     *
    ...
    public function generateSchedule(array $analysis = []): array
    {
        ...
        if (!file_exists($tasksFile)) {
            throw new RuntimeException("tasks.json not found at: {$tasksFile}");
        }
        
        $tasksData = json_decode(file_get_contents($tasksFile), true);
        if (!$tasksData || !isset($tasksData['tasks'])) {
            throw new RuntimeException("Invalid tasks.json format");
        }
